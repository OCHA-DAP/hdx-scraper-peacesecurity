# Collector for peacesecurity Datasets
...
This script connects to the [Peace and Security API](https://psdata.un.org/) and extracts data and metadata from all endpoints creating datasets in HDX. It makes ~100 reads from the data hub and ~100 read/writes (API calls) to HDX in a one hour period. It creates ~100 temporary files each a few Kb which it uploads into HDX. It is run every day.
 ...
 **hdx-scraper-peacesecurity** as specified in the parameter
 *user_agent_lookup*.
...
```shell
    pip install .
    python -m hdx.scraper.peacesecurity
```
